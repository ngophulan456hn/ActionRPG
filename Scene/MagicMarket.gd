...
onready var player = $Rooms/Player
onready var camera2D = $Camera2D
onready var loadingScreen = $CanvasLayer/LoadingScreen

func _ready():
	randomize()
	var last_location = PlayerStats.get_last_location().market
...
func _on_ExitMagicMarket_body_entered(body):
	if body.name == "Player":
		loadingScreen.visible = true
		loadingScreen.start_loading('market', player.global_position, 'WorldScene')
